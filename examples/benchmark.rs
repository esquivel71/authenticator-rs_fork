...
fn main() {
    ...
    let attempts = ITERATIONS;
    let mut n = attempts;
    let mut durations_sign = Vec::with_capacity(n);
    let mut total_time_sign = 0f64;
    let assertion_object = loop {
        println!("Start benchmark! ({})", attempts-n+1);
        let start = OffsetDateTime::now_utc();
        let assertion_object = sign(ctap_args.clone(), &mut manager, status_tx.clone(), timeout_ms);
        let end = OffsetDateTime::now_utc();
        println!("End benchmark! ({})", attempts-n+1);
        let duration = (end - start).as_seconds_f64();
        durations_sign.push(duration);
        n -= 1;
        total_time_sign += duration;
        if n == 0 {
            break assertion_object;
        }
    };
    ...
    println!("Benchmark sign (get assertion): {:?}", durations_sign);
    
    println!("Average sign (get assertion) time: {:?}\n", (total_time_sign / durations_sign.len() as f64));
